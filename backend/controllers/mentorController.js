...
const getMentorDashboard = async (req, res) => {
    try {
        ...
        const studentCount = await safeQuery(`SELECT COUNT(*) as count FROM students WHERE mentor_id = ? AND status NOT IN ('rejected', 'inactive') AND (mentorship_completed = 0 OR mentorship_completed IS NULL) AND (mentorship_paused = 0 OR mentorship_paused IS NULL) ${enrollmentCondition}`, [mentorId], 'studentCount');
...
const getMentorStudents = async (req, res) => {
    try {
        ...
        let whereConditions = `s.status NOT IN ('rejected', 'inactive') AND (s.mentorship_completed = 0 OR s.mentorship_completed IS NULL) AND (s.mentorship_paused = 0 OR s.mentorship_paused IS NULL)
                                 ${enrollmentCondition}`;
    ...
    try {
        ...
        const [rows] = await db.query(`
            SELECT * FROM (
                ...
                ${whereClause.replace(/created_at >= \?/g, 'logs.date >= ?').replace(/created_at <= \?/g, 'logs.date <= ?').replace(/created_at/g, 'logs.created_at')}
                ...
                SELECT 
                    ...
                    CONVERT('DEEP' USING utf8mb4) COLLATE utf8mb4_unicode_ci as session_type,
                    CONVERT(ml.action_details USING utf8mb4) COLLATE utf8mb4_unicode_ci as mentor_notes,
