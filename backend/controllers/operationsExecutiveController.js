...
const addFacultyQualityCheck = async (req, res) => {
    try {
        ...
        await db.query(`
            INSERT INTO aoe_faculty_quality (faculty_id, aoe_id, class_topic, score, remarks)
            VALUES (?, ?, ?, ?, ?)
        `, [faculty_id, aoe_id, class_topic, score, remarks]);
...
const getFacultyReplacements = async (req, res) => {
    try {
        const [rows] = await db.query(`
            ...
            FROM aoe_faculty_replacements r
...
const addFacultyReplacement = async (req, res) => {
    try {
        ...
        await db.query(`
            INSERT INTO aoe_faculty_replacements (faculty_id, aoe_id, reason)
            VALUES (?, ?, ?)
        `, [faculty_id, aoe_id, reason]);
...
const addEscalation = async (req, res) => {
    try {
        ...
        await db.query(`
            INSERT INTO aoe_escalations (student_id, aoe_id, issue_type, description, priority)
            VALUES (?, ?, ?, ?, ?)
        `, [student_id || null, aoe_id, issue_type, description, priority || 'medium']);
