...
const getDashboard = async (req, res) => {
    try {
        ...
        const [[{ totalStudents }]] = await db.query(`
            ...
            FROM students s 
            WHERE EXISTS (SELECT 1 FROM faculty_schedules fs WHERE (fs.is_deleted IS NULL OR fs.is_deleted = 0) AND fs.student_id = s.id AND fs.faculty_id = ?)
        `, [facultyId]);
...
const getStudentProfile = async (req, res) => {
    try {
        ...
        const [check] = await db.query(`
            ...
            WHERE s.id = ? 
              AND EXISTS (SELECT 1 FROM faculty_schedules fs WHERE (fs.is_deleted IS NULL OR fs.is_deleted = 0) AND fs.student_id = s.id AND fs.faculty_id = ?)
        `, [studentId, facultyId]);
