    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <button
                ...
                className="flex items-center gap-2 text-slate-400 hover:text-blue-600 font-black text-[10px] uppercase tracking-widest transition-colors mb-6"
            >
            ...
            <div className="bg-white p-10 rounded-[3rem] shadow-xl shadow-slate-200 border border-slate-50 flex flex-col md:flex-row gap-10 items-center md:items-start relative overflow-hidden">
                ...
                <div className="flex-1 text-center md:text-left relative z-10">
                    ...
                    <div className="flex flex-wrap justify-center md:justify-start gap-4 mb-8">
                        <span className="px-5 py-2 bg-blue-600 text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-lg shadow-blue-100 italic">
                            {student.course}
                        </span>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
                        ...
                        <div>
                            ...
                            <p className="text-base font-bold text-blue-600">{student.next_installment_date ? new Date(student.next_installment_date).toLocaleDateString() : 'Pending'}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                {[
                    ...
                    <button
                        ...
                        className={`
                            ...
                            ${activeTab === tab.id ? 'bg-white text-blue-600 shadow-lg' : 'text-slate-500 hover:text-slate-800'}
                        `}
                    >
                        {tab.icon} {tab.label}
                    </button>
            ...
            <div className="bg-white p-10 rounded-[3rem] shadow-xl shadow-slate-200 border border-slate-50 min-h-[400px]">
                {activeTab === 'info' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                        <section className="space-y-6">
                            <h3 className="text-lg font-black text-slate-900 border-b-2 border-blue-50 pb-2 flex items-center gap-3">
                                <CheckSquare size={20} className="text-blue-600" /> Academic Context
                            </h3>
                        ...
                        <section className="space-y-6">
                            <h3 className="text-lg font-black text-slate-900 border-b-2 border-blue-50 pb-2 flex items-center gap-3">
                                <MessageSquare size={20} className="text-blue-600" /> Enrollment Notes
                            </h3>
                ...
                {activeTab === 'timetable' && (
                    <div className="space-y-6">
                        <h3 className="text-lg font-black text-slate-900 border-b-2 border-blue-50 pb-2">Academic Roadmap (Sessions)</h3>
                                ...
                                <tbody>
                                    {student.timetable.map((session) => (
                                        <tr key={session.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="py-6 px-4 font-bold text-slate-700 text-sm italic">{session.chapter_topic || 'Pending Assignment'}</td>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                        <section className="space-y-6">
                            <h3 className="text-lg font-black text-slate-800 flex items-center gap-3">
                                <History size={20} className="text-blue-600" /> Session History Logs
                            </h3>
                            <div className="space-y-4">
                                {student.studentLogs.map((log) => (
                                    <div key={log.id} className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 hover:shadow-lg transition-all relative overflow-hidden group">
                                        <div className="absolute top-0 right-0 w-20 h-20 bg-blue-100 rounded-full -mr-10 -mt-10 opacity-20 group-hover:scale-150 transition-transform"></div>
                                        <div className="flex justify-between items-start mb-4 relative z-10">
                                            <span className="text-[10px] font-black bg-blue-600 text-white px-3 py-1 rounded-full uppercase">Session #{log.session_number}</span>
                                        ...
                                        <div className="mb-4 space-y-2 relative z-10">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-slate-600 italic line-clamp-2">{log.weak_area_identified || 'N/A'}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex flex-wrap gap-4 relative z-10 pt-2 border-t border-slate-100">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-[10px] font-bold text-blue-600">{log.concept_confidence}/5</span>
                                            </div>
                                            <div className="flex flex-col">
                                        ...
                                        {log.screenshot_url && (
                                            <a href={log.screenshot_url} target="_blank" rel="noopener noreferrer" className="block mt-4 text-[10px] font-black text-blue-500 hover:underline uppercase tracking-wide">View Screenshot Evidence</a>
                                        )}
                                    </div>
                                ))}
                                    ...
                                    <div key={log.id} className="p-6 bg-white rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-md transition-all relative overflow-hidden">
                                        ...
                                        <div className="mb-3 space-y-2 relative z-10">
                                            <div>
                                                ...
                                                <p className="text-xs text-slate-500 italic line-clamp-2">{log.topics_covered}</p>
                                            </div>
                                            <div>
