...
import { 
    ...
    Trash2, CheckCircle2, AlertCircle, ShieldAlert,
    Trash, Eye, RefreshCw
} from 'lucide-react';
            ...
            <header className="bg-white p-10 md:p-14 rounded-[3.5rem] border border-slate-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-10">
                ...
                <div className="flex gap-4">
                    <button 
                        ...
                        className="p-5 bg-slate-50 rounded-[1.5rem] border border-slate-100 text-slate-600 hover:bg-white hover:shadow-lg transition-all"
                    >
                        <RefreshCw size={20} />
                    </button>
                    <button 
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {loading ? (
                                ...
                                filteredLogs.map((log) => (
                                    <tr key={log.id} className={`hover:bg-slate-50/50 transition-all group ${log.is_read ? 'opacity-60' : ''}`}>
