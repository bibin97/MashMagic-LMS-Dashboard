...
import { Calendar, Target, Users, CheckCircle2, ShieldAlert, DownloadCloud } from 'lucide-react';
...
const DailyMentorHeadReport = () => {
  ...
  return <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
 ...
 <input type="date" value={filterDate} onChange={e => setFilterDate(e.target.value)} max={new Date().toISOString().split('T')[0]} className="bg-white border border-slate-200 text-slate-700 text-sm font-bold rounded-2xl pl-12 pr-4 py-3 outline-none focus:ring-4 focus:ring-[#008080] transition-all hover:border-[#008080]" />
 </div>
 </div>
 </header>
 ...
 </tr> : reportData.length === 0 ? <tr>
 <td colSpan="4" className="p-12 text-center text-slate-600 font-bold">
 No Mentor Heads found for this date.
 </td>
 </tr> : reportData.map((row, idx) => <tr key={idx} className="border-b last:border-b-0 border-slate-50 hover:bg-slate-50/50 transition-colors group"><td className="p-6 text-sm font-black text-slate-400 border-b border-slate-50">{idx + 1}</td>
 ...
 {globalCheckedStudents.length > 0 ? <ul className="divide-y divide-emerald-50/50">
 {globalCheckedStudents.map(student => <li key={student.id} className="py-4 hover:bg-emerald-50/30 transition-colors flex flex-col gap-1 rounded-xl px-4 group">
 ...
 </ul> : <div className="h-full flex flex-col items-center justify-center opacity-40">
 ...
 <div className="flex-1 overflow-y-auto custom-scrollbar px-2">
 {globalRemainingStudents.length > 0 ? <ul className="divide-y divide-rose-50/50">
 {globalRemainingStudents.map(student => <li key={student.id} className="py-4 hover:bg-rose-50/30 transition-colors flex flex-col gap-1 rounded-xl px-4 group">
