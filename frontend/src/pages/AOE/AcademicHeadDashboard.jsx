...
const AcademicHeadDashboard = () => {
 ...
 <div className="flex items-center gap-5">
   ...
   <button 
     onClick={async () => {
       const load = toast.loading("Deduplicating...");
       try {
         const res = await api.post('/aoe/deduplicate-students');
         toast.success(res.data.message || "Deduplicated!", { id: load });
         window.location.reload();
       } catch (e) {
         toast.error(e.response?.data?.message || "Error", { id: load });
       }
     }}
     className="bg-rose-500 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase"
   >
     Remove Duplicates
   </button>
