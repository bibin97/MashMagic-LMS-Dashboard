...
const StudentCheckTracker = () => {
 ...
 <span className="text-[10px] font-black uppercase tracking-widest text-slate-600 ml-2">Sort By:</span>
 <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="bg-white border-none rounded-xl px-4 py-2 text-xs font-bold text-slate-700 outline-none cursor-pointer focus:ring-2 focus:ring-[#008080]/20 shadow-sm">
 ...
 {getSortedStudents().map((student, index) => {
              ...
              if (checkCount === 1) {
                ...
                badge = <span className="inline-flex items-center justify-center w-6 h-6 ml-2 rounded-full bg-rose-500 text-white text-[10px] font-black shadow-sm">
 {checkCount}
 </span>;
              }
              return <tr key={student.student_id} className={rowClass}><td className="p-6 text-sm font-black text-slate-400 border-b border-slate-50">{index + 1}</td>
 <td className="p-4 text-sm font-bold flex items-center gap-2">
 {student.student_name}
 {student.onboarding_status === 'pending' && <span className="px-2 py-0.5 bg-rose-50 rounded-md text-[8px] font-black text-rose-600 shadow-sm border border-rose-100 uppercase tracking-widest">
 New
 </span>}
 {badge}
 </td>
