...
const StudentsData = () => {
  ...
  const [comprehensiveData, setComprehensiveData] = useState({ dailyUpdates: [], marks: [], attendance: [] });
              ...
              <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden min-h-[500px]">
                <div className="p-8 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="flex gap-4 p-1 bg-slate-100 rounded-2xl">
                    {[
                      { id: 'activity', label: 'Activity Streams', icon: <ClipboardList size={14} /> },
                      { id: 'marks', label: 'Exam Marks', icon: <TrendingUp size={14} /> },
                      ...
                      <button
                        ...
                        className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                          detailTab === tab.id ? 'bg-white text-teal-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                        }`}
                      >
                        {tab.icon} {tab.label}
                      </button>
                ...
                <div className="p-8">
                  {updatesLoading ? (
                    ...
                    <>
                      {detailTab === 'activity' && (
                        <div className="space-y-6">
                          {(comprehensiveData.dailyUpdates || []).length === 0 ? (
                            ...
                            <div className="relative space-y-8">
                              ...
                              {comprehensiveData.dailyUpdates.map((update, idx) => (
                                <div key={update.id} className="relative flex flex-col md:flex-row gap-6 animate-in slide-in-from-bottom-4 duration-500">
                      ...
                      {detailTab === 'marks' && (
                        <div className="space-y-6">
                          {(comprehensiveData.marks || []).length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-80 text-center p-12 bg-slate-50/50 rounded-[32px] border border-slate-100">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              {comprehensiveData.marks.map(mark => (
                                <div key={mark.id} className="p-6 bg-slate-50/50 rounded-[32px] border border-slate-100 hover:bg-white transition-all">
                      ...
                      {detailTab === 'attendance' && (
                        <div className="space-y-6">
                          {(comprehensiveData.attendance || []).length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-80 text-center p-12 bg-slate-50/50 rounded-[32px] border border-slate-100">
                              ...
                              <table className="w-full text-left border-collapse">
                                ...
                                <tbody>
                                  {comprehensiveData.attendance.map(att => (
                                    <tr key={att.id} className="border-t border-slate-50 hover:bg-slate-50/30 transition-colors">
